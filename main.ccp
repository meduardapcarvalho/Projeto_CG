...
// Função para aplicar o cisalhamento a um conjunto de pontos
void applyShear(std::vector<std::pair<float, float>>& points, float shearX, float shearY) {
    ...
    void applyTransformation(std::vector<std::pair<float, float>>&points, const std::vector<std::pair<char, std::pair<float, float>>>&vectorTransformation) {
        for (const auto& transformation : vectorTransformation) {
...
int main(int argc, char* argv[]) {
    ...
    window = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "Visualizador de Curvas de Bezier");
    if (!window) {
